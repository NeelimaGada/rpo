...
export class SkillserviceService {
  ...
  tabledata(a){
   return this.ht.get("http://192.168.1.137:8087/rpo/rest/skill",{
    headers:{
      'X-Access-Token':a,
 'Content-Type':'application/json'
  ...
  let p = localStorage.getItem("token");
    return this.ht.post("http://192.168.1.137:8087/rpo/rest/skill/"+ r.id,r,{
      headers:{
        'X-Access-Token':p,
   'Content-Type':'application/json'
   }
  ...
  addnewdetails(a){
    let p = localStorage.getItem("token")
    return this.ht.post("http://192.168.1.137:8087/rpo/rest/skill",a,{
      headers:{
        'X-Access-Token':p,
   'Content-Type':'application/json'
   }
